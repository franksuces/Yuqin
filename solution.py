...
def naked_twins(values):
    ...
    pairs=[[x1,x2] for x1 in twins for x2 in peers[x1] if (values[x1]==values[x2])]
    for x1,x2 in pairs:
        # because the list and dict types are not hashable, I have to use set, that's the only choice
        set1=set(peers[x1])
        set2=set(peers[x2])
        xset=set1.intersection(set2)
        for rest in xset:
            digit=values[x1]
            values=assign_value(values,rest,values[rest].replace(digit,''))
    return values
